...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🌍 市場氣氛","📊 個股掃描","📐 回撤計算","📈 技術圖表","🎯 四維撈底評分","📋 信號追蹤"])
...
with tab1:
    ...
    st.markdown("### 📊 全球宏觀指標")
    kpi_items = [("VIX","😱 恐慌指數"),("VVIX","🌊 波動之波動"),("SPX","🇺🇸 標普500"),("HSI","🇭🇰 恒生指數"),
                 ("US10Y","🏦 美債10年息"),("DXY","💵 美元指數"),("HYG","📉 高收益債"),("VHSI","🇭🇰 港股波幅")]
    for row_items in [kpi_items[:4], kpi_items[4:]]:
        ...
        for i,(key,label) in enumerate(row_items):
            ...
            arrow = "▲" if chg>=0 else "▼"
            emoji = label.split()[0]; name = ' '.join(label.split()[1:])
            with cols_kpi[i]:
                st.markdown(f"<div class='metric-card'><div style='font-size:1em'>{emoji}</div><div style='color:#8b949e;font-size:0.7em'>{name}</div><div style='font-size:1.1em;font-weight:bold;color:#e6edf3;margin:4px 0'>{val:.2f}</div><div style='color:{color};font-size:0.82em'>{arrow} {chg:+.2f}%</div><div style='color:#8b949e;font-size:0.68em'>52W:{pct:.0f}%</div></div>", unsafe_allow_html=True)
    st.divider()
    st.markdown("### 🎯 撈底機會總評")
    g_col1,g_col2,g_col3 = st.columns(3)
    vix_score = 80 if vix_now>=30 else (60 if vix_now>=25 else (20 if vix_now<=15 else 40))
    def make_gauge(score, title):
        gc, gt = (C_GREEN,"🔥 極佳撈底視窗") if score>=70 else ((C_ORANGE,"⚠️ 謹慎撈底機會") if score>=55 else ((C_GREY,"😐 市場中性") if score>=40 else (C_RED,"😎 市場貪婪風險")))
        fig = go.Figure(go.Indicator(mode="gauge+number", value=score, title={"text":f"{title}<br><span style='font-size:0.7em;color:{gc}'>{gt}</span>"}, number={"font":{"color":gc,"size":40},"suffix":"/100"}, gauge={"axis":{"range":[0,100]},"bar":{"color":gc,"thickness":0.25},"bgcolor":"#161b22","bordercolor":"#30363d","steps":[{"range":[0,25],"color":"#1a1a2e"},{"range":[25,45],"color":"#1c1a00"},{"range":[45,65],"color":"#161b22"},{"range":[65,80],"color":"#0d2818"},{"range":[80,100],"color":"#0d3318"}],"threshold":{"line":{"color":"#ffffff","width":3},"thickness":0.8,"value":score}}))
...
with tab2:
    ...
    else: tickers = [x.strip().upper() for x in custom_input.split("\n") if x.strip()] or US_WATCHLIST
    macro_now = fetch_macro()
    vix_now = macro_now.get("VIX",{}).get("val",20) if isinstance(macro_now.get("VIX"),dict) else 20
    if vix_now>=30: fc,fi,fl,auto_min_mid = C_GREEN,"🔥",f"VIX {vix_now:.1f} 極度恐慌 — 只顯示最強信號（建議中線分≥60）",60
    elif vix_now>=25: fc,fi,fl,auto_min_mid = C_ORANGE,"⚠️",f"VIX {vix_now:.1f} 高波動市場 — 建議只操作中線分≥50的股票",50
    elif vix_now<=15: fc,fi,fl,auto_min_mid = C_RED,"😎",f"VIX {vix_now:.1f} 市場過度貪婪 — 注意追高風險，降低倉位",0
    else: fc,fi,fl,auto_min_mid = C_GREY,"😐",f"VIX {vix_now:.1f} 市場中性 — 正常操作",0
    st.markdown(f"<div style='background:#161b22;border-left:4px solid {fc};border-radius:8px;padding:12px 16px;margin-bottom:12px'><span style='font-size:1.1em'>{fi}</span> <span style='color:{fc};font-weight:bold'>市場氣氛濾網</span>: <span style='color:#e6edf3'>{fl}</span></div>", unsafe_allow_html=True)
    effective_min_mid = max(min_mid, auto_min_mid)
    if auto_min_mid>0 and auto_min_mid>min_mid: st.caption(f"💡 VIX濾網已自動將最低中線分提升至 {auto_min_mid}")
    st.subheader(f"📊 個股掃描 — {market} ({len(tickers)} 隻)")
    with st.spinner(f"正在並行下載 {len(tickers)} 隻股票數據..."): data_map = fetch_multiple(tickers, period="2y")
    rows = []
    for tk in tickers:
        df = data_map.get(tk)
        ...
        close_v = float(df["close"].iloc[-1]); hi52 = get_52w_high(df)
        chg1d = (close_v-float(df["close"].iloc[-2]))/float(df["close"].iloc[-2])*100
        vol_rat = float(df["volume"].iloc[-1])/(float(df["volume"].rolling(20).mean().iloc[-1]) or 1)
        ...
        rows.append({"代碼":tk,"現價":round(close_v,3),"1日漲跌%":round(chg1d,2),"52周高":round(hi52,3),"距高位%":round((close_v-hi52)/hi52*100,1),"量比":round(vol_rat,2),"周線RSI":round(rsi_w_v,1),"短線分":short_s,"中線分":mid_s,"信號":label,"_type":stype,"VIX環境":vix_env,"觸發指標":"、".join(sigs) if sigs else "—","_drop":drop_levels(hi52),"_fib":fib_levels(swing_lo,hi52),"_df":df,"cmf":cmf_val,"vwap":vwap_val,"vol_z":vol_z,"resonance":resonance,"oversold_count":oversold_count})
    st.markdown("### 🚨 低位大成交量提示（距20日低點≤5% 且 Z-score≥2.0 且 收陽線）")
    volume_alerts = []
    for r in rows:
        df = r["_df"]
        if df is None or len(df)<20: continue
        close_now = r["現價"]; recent_low = float(df["low"].iloc[-20:].min())
        ...
        if pct_above_low <= 5 and r["vol_z"] >= 2.0 and is_positive:
            volume_alerts.append({"代碼":r["代碼"],"現價":close_now,"近期低點":round(recent_low,3),"距低點%":round(pct_above_low,1),"量比":r["量比"],"信號":r["信號"],"Z-score":r["vol_z"]})
    if volume_alerts:
        ...
        for i, alert in enumerate(volume_alerts):
            with alert_cols[i%3]:
                st.markdown(f"<div class='volume-alert'><b style='font-size:1.2em;'>{alert['代碼']}</b><br>現價：<b>{alert['現價']}</b><br>近期低點：{alert['近期低點']}<br>距低點：<b>{alert['距低點%']}%</b><br>量比：<b style='color:#3fb950;'>{alert['量比']}x</b><br>Z-score：<b>{alert['Z-score']}</b><br>信號：{alert['信號']}</div>", unsafe_allow_html=True)
    else: st.info("目前沒有股票符合「低位 + 大成交量 + 收陽線」的條件。")
    st.divider()
    if resonance_filter != "全部": rows = [r for r in rows if r["resonance"] == resonance_filter]
    filtered = [r for r in rows if r["信號"] in filter_sig and r["短線分"]>=min_short and r["中線分"]>=effective_min_mid]
    ...
    if filtered:
        ...
        fig_bar = px.bar(df_plot.melt(id_vars="代碼",value_vars=["短線分","中線分"]), x="代碼",y="value",color="variable",barmode="group",color_discrete_map={"短線分":"#388bfd","中線分":C_GREEN},height=260)
        fig_bar.update_layout(paper_bgcolor=C_BG,plot_bgcolor=C_BG,font=dict(color="#e6edf3"),margin=dict(l=5,r=5,t=10,b=5))
        st.plotly_chart(fig_bar, use_container_width=True)
        sort_col1,_ = st.columns([2,4])
        with sort_col1: sort_by = st.selectbox("排序方式", ["總分（短+中）","短線分","中線分","量比","距高位%","周線RSI"], key="sort_by")
        sort_map = {"總分（短+中）": lambda x: x["短線分"]+x["中線分"],"短線分": lambda x: x["短線分"],"中線分": lambda x: x["中線分"],"量比": lambda x: x["量比"],"距高位%": lambda x: -x["距高位%"],"周線RSI": lambda x: x["周線RSI"]}
        sorted_filtered = sorted(filtered, key=sort_map[sort_by], reverse=sort_by not in ["距高位%"])
        for r in sorted_filtered:
            badge_class = signal_badge(r['信號'])
            weekly_warn = " ⚠️周線仍強" if r.get("周線RSI",50)>60 else ""
            resonance_class = "resonance-strong" if r['resonance']=="強" else ("resonance-medium" if r['resonance']=="中" else "resonance-weak")
            with st.expander(f"<span class='signal-badge {badge_class}'>{r['信號']}</span> {r['代碼']} 現價 {r['現價']} ({r['1日漲跌%']:+.1f}%) ｜ 短線:{r['短線分']} 中線:{r['中線分']} <span class='{resonance_class}'>{r['resonance']}共振</span>{weekly_warn}"):
                col_a,col_b,col_c,col_d = st.columns(4)
                col_a.metric("CMF (資金流)", f"{r['cmf']:.3f}"); col_a.caption("💰 資金流入" if r['cmf']>0 else "⚠️ 資金流出")
                col_b.metric("VWAP (均價)", f"{r['vwap']:.2f}"); col_b.caption("股價在均價之上" if r['現價']>r['vwap'] else "股價在均價之下")
                col_c.metric("成交量 Z-score", f"{r['vol_z']:.2f}"); col_c.caption("🔥 極度放量" if r['vol_z']>2.0 else ("📈 明顯放量" if r['vol_z']>1.0 else "正常量"))
                col_d.metric("共振指標數", f"{r['oversold_count']}/4"); col_d.caption(f"{r['resonance']}共振（RSI/KDJ/CCI/W%R）")
                st.markdown(f"**觸發指標：** {r['觸發指標']}")
                df_vol = r["_df"]
                if df_vol is not None and len(df_vol)>=20:
                    vol_s = df_vol["volume"]; vol_m20 = vol_s.rolling(20).mean(); vol_ma20v = float(vol_m20.iloc[-1])
                    is_shrinking = all(float(vol_s.iloc[-i])<vol_ma20v*0.85 for i in range(1,4))
                    is_expanding = float(vol_s.iloc[-1])>vol_ma20v*1.5
                    is_diverging = float(vol_s.iloc[-1])<vol_ma20v*0.7 and float(df_vol["close"].iloc[-1])<float(df_vol["close"].iloc[-2])
                    vol_tags = []
                    if is_shrinking: vol_tags.append("📉 縮量整理")
                    if is_expanding: vol_tags.append("📈 放量介入")
                    if is_diverging: vol_tags.append("🔵 量縮價跌")
                    st.markdown("**量能形態：** " + (" ".join(vol_tags) if vol_tags else "正常"))
                d1,d2 = st.columns(2)
                with d1:
                    drop_df = pd.DataFrame(list(r["_drop"].items()), columns=["回撤","價位"])
                    drop_df["狀態"] = drop_df["價位"].apply(lambda x: "◀ 當前附近" if abs(x-r["現價"])/r["現價"]<0.03 else "")
                    st.dataframe(drop_df, use_container_width=True, hide_index=True)
                with d2:
                    fib_df = pd.DataFrame(list(r["_fib"].items()), columns=["比率","價位"])
                    fib_df["狀態"] = fib_df["價位"].apply(lambda x: "◀ 當前附近" if abs(x-r["現價"])/r["現價"]<0.03 else "")
                    st.dataframe(fib_df, use_container_width=True, hide_index=True)
                st.markdown(f"**🎯 目標+20%：`{round(r['現價']*1.2,3)}`  ｜  止損-8%：`{round(r['現價']*0.92,3)}`**")
    st.divider()
    st.subheader("📋 全部股票列表")
    if rows: st.dataframe(pd.DataFrame([{k:v for k,v in r.items() if not k.startswith("_")} for r in rows]).sort_values("短線分",ascending=False), use_container_width=True, hide_index=True)
...
with tab4:
    ...
    if df_ch is not None and len(df_ch)>30:
        close_ch = df_ch["close"]; rsi_ch = calc_rsi(close_ch); rsi_w_ch = calc_rsi(close_ch,70)
        ...
        for ma,mc,nm in [(sma20,"#f0883e","MA20"),(sma60,C_BLUE,"MA60"),(sma200,C_PURPLE,"MA200")]: fig_tech.add_trace(go.Scatter(x=df_ch.index,y=ma,mode="lines",line=dict(color=mc,width=1.2),name=nm),row=1,col=1)
        fig_tech.add_trace(go.Scatter(x=df_ch.index,y=bb_up,mode="lines",line=dict(color=C_GREY,dash="dot"),showlegend=False),row=1,col=1)
        fig_tech.add_trace(go.Scatter(x=df_ch.index,y=bb_dn,mode="lines",line=dict(color=C_GREY,dash="dot"),fill="tonexty",fillcolor="rgba(139,148,158,0.05)",showlegend=False),row=1,col=1)
        vol_colors = [C_GREEN if df_ch["close"].iloc[i]>=df_ch["open"].iloc[i] else C_RED for i in range(len(df_ch))]
        fig_tech.add_trace(go.Bar(x=df_ch.index,y=df_ch["volume"],marker_color=vol_colors,showlegend=False),row=2,col=1)
        ...
        fig_tech.add_trace(go.Bar(x=df_ch.index,y=hist_ch,marker_color=hist_colors,showlegend=False),row=4,col=1)
        ...
        fig_tech.add_hline(y=-20,line_dash="dash",line_color=C_RED,row=7,col=1); fig_tech.add_hline(y=-80,line_dash="dash",line_color=C_GREEN,row=7,col=1)
        ...
        close_v = float(df_ch["close"].iloc[-1]); rsi_now = float(calc_rsi(df_ch["close"]).iloc[-1]); rsi_w_now = float(calc_rsi(df_ch["close"],70).iloc[-1])
        c1,c2,c3,c4,c5,c6 = st.columns(6)
        c1.metric("短線評分",f"{short_s}/100"); c2.metric("中線評分",f"{mid_s}/100"); c3.metric("信號",label); c4.metric("日線RSI",f"{rsi_now:.1f}"); c5.metric("周線RSI",f"{rsi_w_now:.1f}"); c6.metric("目標+20% / 止損-8%",f"{round(close_v*1.2,3)} / {round(close_v*0.92,3)}")
...
with tab5:
    ...
    def technical_detail_score(df):
        ...
        close=df["close"]; rsi=calc_rsi(close).iloc[-1]; K,D,_=calc_kdj(df); k,d=K.iloc[-1],D.iloc[-1]; cci=calc_cci(df).iloc[-1]; wr=calc_wr(df).iloc[-1]
        detail={}
        ...
        return min(sum(v[0] for v in detail.values()),100), detail

    def fund_flow_detail(df):
        if df is None or len(df)<20: return 0,{}
        close,volume=df["close"],df["volume"]; mfi_series=calc_mfi(df); mfi_now=float(mfi_series.iloc[-1]) if not pd.isna(mfi_series.iloc[-1]) else 50
    ...
    def four_dimension_score(ticker):
        ...
        val_score=50; val_detail="無數據(預設50分)"; pe_percentile=None
        ...
        hi52=get_52w_high(df); current_price=float(df["close"].iloc[-1])
        ...
        if capital_flow is not None and not capital_flow.empty:
            latest=capital_flow.iloc[-1]; inflow=latest.get('in_flow',0) if 'in_flow' in latest else 0
            if inflow>0: capital_bonus=10; capital_detail=f"主力流入 {inflow:.0f}萬"
            else: capital_detail="主力流出"
        fund_total=min(fund_total+capital_bonus,100)
        macro=fetch_macro(); vix_v=macro.get("VIX",{}).get("val",20) if isinstance(macro.get("VIX"),dict) else 20
        w=get_dynamic_weights(vix_v)
        total=round(w["tech"]*tech_total + w["val"]*val_score + w["dd"]*dd_score + w["fund"]*fund_total,1)
        confidence="高信心" if total>=80 else ("中等信心" if total>=60 else "低信心")
        return {"ticker":ticker,"name":name,"price":round(current_price,2),"total_score":total,"confidence":confidence,
                "tech_total":tech_total,"rsi_score":rsi_score,"kdj_score":kdj_score,"cci_score":cci_score,"wr_score":wr_score,
                "val_score":val_score,"val_detail":val_detail,"pe_percentile":pe_percentile,"drawdown":drawdown,"dd_score":dd_score,
                "fund_total":fund_total,"downvol_score":downvol_score,"mfi_score":mfi_score,"mfi_trend_score":mfi_trend_score,
                "capital_detail":capital_detail,"hi52":hi52,"weights":w,"vix":vix_v}
    ...
    if scan_list:
        ...
        if results:
            ...
            for r in results:
                if r["total_score"]>=70: log_signal(r["ticker"],r["total_score"],r["confidence"],r["price"],datetime.now().strftime("%Y-%m-%d"))
            st.markdown("---")
            st.markdown("### 📋 四維評分詳細細項表")
            w_disp=results[0]["weights"]
            st.caption(f"VIX={results[0]['vix']:.1f}，權重：技術{w_disp['tech']:.0%}/估值{w_disp['val']:.0%}/回調{w_disp['dd']:.0%}/資金{w_disp['fund']:.0%}")
            detailed_data=[]
            for r in results:
                detailed_data.append({"代碼":r["ticker"],"名稱":r["name"],"現價":r["price"],"總分":r["total_score"],"信心":r["confidence"],
                    "技術總分":r["tech_total"],"RSI":r["rsi_score"],"KDJ":r["kdj_score"],"CCI":r["cci_score"],"WR":r["wr_score"],
                    "估值":r["val_score"],"PE百分位":f"{r['pe_percentile']*100:.0f}%" if r['pe_percentile'] else "N/A",
                    "回撤%":r["drawdown"],"回撤得分":r["dd_score"],"資金總分":r["fund_total"],
                    "大跌量":r["downvol_score"],"MFI":r["mfi_score"],"MFI趨勢":r["mfi_trend_score"],"富途資金":r["capital_detail"]})
        ...
        else: st.warning("沒有找到有效數據。")
    else: st.info("👆 點擊「掃描當前觀察名單」即自動分析已選市場的所有股票。")
    st.divider()
...
with tab6:
    ...
    except FileNotFoundError: st.info("信號記錄檔案不存在，將在第一次掃描時建立。")
