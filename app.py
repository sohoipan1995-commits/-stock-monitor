...
def score_stock(df, market_state="neutral"):
    ...
    if market_state == "bear_high_vol":
        short_score = int(short_score * 1.1)
...
def classify_market_state():
    try:
        ...
        elif ret_60 < -5 and vix <= 25: return "bear_low_vol", ret_60, volatility
        elif ret_60 > 5 and vix > 25: return "bull_high_vol", ret_60, volatility
        elif ret_60 > 5 and vix <= 25: return "bull_low_vol", ret_60, volatility
        else: return "neutral", ret_60, volatility
    except:
        return "unknown", 0, 0
...
def calculate_position(price, stop_loss, account_size=100000, risk_pct=0.02):
    ...
    if per_share_risk <= 0: return 0, risk_amount
...
with tab1:
        ...
        for i,(key,label) in enumerate(row_items):
            ...
            with cols_kpi[i]:
                st.markdown(
                    ...
                    f"<div style='font-size:1.1em;font-weight:bold;color:#e6edf3;margin:4px 0'>{val:.2f}</div>"
                    f"<div style='color:{color};font-size:0.82em'>{arrow} {chg:+.2f}%</div>"
                    f"<div style='color:#8b949e;font-size:0.68em'>52W:{pct:.0f}%</div>"
                    f"</div>",
                    unsafe_allow_html=True)
    ...
    def make_gauge(score, title):
        ...
        fig = go.Figure(go.Indicator(
            ...
            gauge={"axis":{"range":[0,100]},"bar":{"color":gc,"thickness":0.25},
                ...
                "threshold":{"line":{"color":"#ffffff","width":3},"thickness":0.8,"value":score}}))
...
with tab2:
    ...
    if vix_now>=30:
        fc=C_GREEN; fi="🔥"; fl=f"VIX {vix_now:.1f} 極度恐慌 — 只顯示最強信號（建議中線分≥60）"; auto_min_mid=60
    elif vix_now>=25:
        fc=C_ORANGE; fi="⚠️"; fl=f"VIX {vix_now:.1f} 高波動市場 — 建議只操作中線分≥50的股票"; auto_min_mid=50
    elif vix_now<=15:
        fc=C_RED; fi="😎"; fl=f"VIX {vix_now:.1f} 市場過度貪婪 — 注意追高風險，降低倉位"; auto_min_mid=0
    else:
        fc=C_GREY; fi="😐"; fl=f"VIX {vix_now:.1f} 市場中性 — 正常操作"; auto_min_mid=0
    ...
    st.markdown(
        ...
        f"<span style='color:#e6edf3'>{fl}</span></div>", unsafe_allow_html=True)
    ...
    for tk in tickers:
        ...
        rows.append({
            "代碼":tk,"現價":round(close_v,3),"1日漲跌%":round(chg1d,2),
            "52周高":round(hi52,3),"距高位%":round((close_v-hi52)/hi52*100,1),
            "量比":round(vol_rat,2),"周線RSI":round(rsi_w_v,1),
            "短線分":short_s,"中線分":mid_s,"信號":label,"_type":stype,"VIX環境":vix_env,
            "觸發指標":"、".join(sigs) if sigs else "—",
            "_drop":drop_levels(hi52),"_fib":fib_levels(swing_lo,hi52),
    ...
    for r in rows:
        ...
        if pct_above_low <= 5 and r["vol_z"] >= 2.0 and is_positive:
            volume_alerts.append({
                "代碼": r["代碼"], "現價": close_now, "近期低點": round(recent_low,3),
                "距低點%": round(pct_above_low,1), "量比": r["量比"], "信號": r["信號"], "Z-score": r["vol_z"]
            })
    if volume_alerts:
    ...
    if filtered:
        ...
        fig_bar.update_layout(paper_bgcolor=C_BG,plot_bgcolor=C_BG,font=dict(color="#e6edf3"),legend_title="",margin=dict(l=5,r=5,t=10,b=5))
        ...
        sort_key_map = {
            "總分（短+中）": lambda x: x["短線分"]+x["中線分"], "短線分": lambda x: x["短線分"],
            "中線分": lambda x: x["中線分"], "量比": lambda x: x["量比"],
            "距高位%": lambda x: -x["距高位%"], "周線RSI": lambda x: x["周線RSI"],
        }
        ...
        for r in sorted_filtered:
            ...
            with st.expander(
                ...
                with col_a:
                    ...
                    if r['cmf'] > 0.2: st.caption("💰 資金明顯流入 → 主力吸籌，可跟進")
                    elif r['cmf'] > 0: st.caption("✅ 資金輕微流入 → 等待進一步確認")
                    elif r['cmf'] > -0.2: st.caption("⚠️ 資金輕微流出 → 小心再跌一段")
                    else: st.caption("❌ 資金明顯流出 (派發) → 暫不進場")
                with col_b:
                    st.metric("VWAP (均價)", f"{r['vwap']:.2f}")
                    if r['現價'] > r['vwap']: st.caption("📈 股價在均價之上 (強勢) → 買盤願追價")
                    else: st.caption("📉 股價在均價之下 (弱勢) → 等站回 VWAP")
                with col_c:
                    st.metric("成交量 Z-score", f"{r['vol_z']:.2f}")
                    if r['vol_z'] > 2.0: st.caption("🔥 極度放量 → 恐慌拋售或主力吸籌")
                    elif r['vol_z'] > 1.0: st.caption("📈 明顯放量 → 有資金關注")
                    elif r['vol_z'] < -1.5: st.caption("❄️ 極度縮量 → 等待放量信號")
                    else: st.caption("正常量")
                with col_d:
                    st.metric("共振指標數", f"{r['oversold_count']}/4")
                    triggers_desc = "RSI, KDJ, CCI, W%R"
                    if r['resonance'] == "強": st.caption(f"⚡ 強共振 ({triggers_desc}) → 進場信心高")
                    elif r['resonance'] == "中": st.caption(f"🔹 中共振 ({triggers_desc}) → 可小量試單")
                    elif r['resonance'] == "弱": st.caption(f"▪️ 弱共振 ({triggers_desc}) → 等待更多確認")
                    else: st.caption(f"— 無共振 ({triggers_desc}) → 暫觀望")
                ...
                if df_vol is not None and len(df_vol)>=20:
                    vol_s = df_vol["volume"]; vol_m20 = vol_s.rolling(20).mean()
                    ...
                    fig_mini = go.Figure(go.Bar(x=dates_5,y=vr_list,marker_color=bar_c5,
                        text=[f"{v:.1f}x" for v in vr_list], textposition="outside",textfont=dict(color="#e6edf3",size=10)))
                    ...
                    fig_mini.update_layout(title="近5日量比",height=200,paper_bgcolor=C_BG,plot_bgcolor=C_BG,
...
with tab3:
    ...
    if st.button("🔍 計算",type="primary"):
        ...
        if df_c is not None:
            ...
            ca,cb = st.columns(2)
            with ca:
                rows_d = [{"回撤幅度":pct,"目標價位":price,"現價距離":f"{close_now-price:+.2f}",
                    "狀態":"✅ 已達到" if close_now<=price*1.02 else f"還需跌 {abs(close_now-price):.2f}"} for pct,price in drop_levels(hi).items()]
                st.dataframe(pd.DataFrame(rows_d),use_container_width=True,hide_index=True)
            with cb:
                fibs_v = fib_levels(lo,hi)
                rows_f = [{"比率":ratio,"支撐價位":price,"現價距離":f"{close_now-price:+.2f}",
                    "狀態":"◀ 當前附近" if abs(close_now-price)/close_now<0.03 else ("✅ 已跌穿" if close_now<price else f"距離 {close_now-price:+.2f}")} for ratio,price in fibs_v.items()]
...
with tab4:
    ...
    if df_ch is not None and len(df_ch)>30:
        ...
        if rsi_now<35 and 28<=rsi_w_now<=50:
            st.markdown(f"<div style='background:#0d2818;border:1px solid #238636;border-radius:8px;padding:12px;margin-top:8px'><b style='color:{C_GREEN}'>⭐ 雙周期RSI共振信號</b><br>代表中線底部支撐的真實超賣機會。</div>", unsafe_allow_html=True)
    ...
    def four_dimension_score(ticker, watchlist):
        ...
        if pe is not None and pe > 0:
            ...
            if pe_perc is not None:
                ...
                elif pe_perc < 0.25: pe_val_score = 70
                elif pe_perc < 0.5: pe_val_score = 40
                else: pe_val_score = 10
            else:
                if pe < 10: pe_val_score = 90
                elif pe < 15: pe_val_score = 70
                elif pe < 20: pe_val_score = 40
                else: pe_val_score = 10
        ...
        if pb is not None and pb > 0:
            ...
            if pb_perc is not None:
                ...
                elif pb_perc < 0.25: pb_val_score = 70
                elif pb_perc < 0.5: pb_val_score = 40
                else: pb_val_score = 10
            else:
                if pb < 1: pb_val_score = 90
                elif pb < 1.5: pb_val_score = 70
                elif pb < 2.5: pb_val_score = 40
                else: pb_val_score = 10
        ...
        elif pe_val_score is not None:
            val_score = pe_val_score
            val_detail = f"PE {pe:.1f}" + (f"，歷史百分位 {pe_percentile*100:.0f}%" if pe_percentile is not None else "")
        elif pb_val_score is not None:
        ...
        capital_bonus = 0; capital_detail = "未取得(需富途連線)"
    ...
    try:
        ...
        if not df_log.empty:
                ...
                try:
                    ...
                    if df_bt is not None and len(df_bt) > hold_days:
                        ...
                        if len(future_dates) > hold_days:
                            ...
                            backtest_results.append({"ticker": ticker,"進場日": entry_date.strftime("%Y-%m-%d"),
                                "進場價": entry_price,"出場價": round(exit_price, 2),"回報%": round(ret, 2)})
                except Exception:
                    continue
            ...
            if backtest_results:
                ...
                fig_bt = go.Figure(go.Scatter(x=df_bt.index, y=df_bt["累積回報"]*100, mode='lines+markers', name='累積回報', line=dict(color=C_GREEN, width=2)))
                fig_bt.update_layout(title=f"信號累積回報 (持有{hold_days}天)", height=400, paper_bgcolor=C_BG, plot_bgcolor=C_BG, font=dict(color="#e6edf3"))
    ...
    if st.button("計算部位"):
        ...
        if df is not None:
            ...
            for ratio, price in sorted(fibs.items(), key=lambda x: float(x[0].replace("%", ""))):
                if price < close_v: support_level = price; break
            if support_level is None: support_level = close_v * 0.92
...
with tab8:
    ...
    with mcpe_c1: mcpe_tk = st.text_input("輸入股票代碼 (MCPE)", "0700.HK", key="mcpe_tk").upper()
    with mcpe_c2: mcpe_period = st.selectbox("分析週期", ["1y", "2y", "3y", "5y"], index=1, key="mcpe_period")
    with mcpe_c3: mcpe_dev = st.number_input("轉折靈敏度 (幅度%)", value=5.0, min_value=1.0, max_value=20.0, step=0.5, help="波段回撤多少百分比才確認為新的高/低點。")
    with mcpe_c4:
        ...
        if df_mcpe is not None and len(df_mcpe) > 30:
            def calculate_zigzag(df, deviation_pct):
                ...
                for i in range(1, len(df)):
                    ...
                    if trend == 1:
                        if high > high_p: high_p, high_t = high, idx
                        elif low < high_p * (1 - dev):
                            pivots.append((high_t, high_p, 1)); trend = -1; low_p, low_t = low, idx
                    else:
                        if low < low_p: low_p, low_t = low, idx
                        elif high > low_p * (1 + dev):
                            pivots.append((low_t, low_p, -1)); trend = 1; high_p, high_t = high, idx
            ...
            with chart_col:
                fig_mcpe = go.Figure()
                fig_mcpe.add_trace(go.Candlestick(x=df_mcpe.index, open=df_mcpe["open"], high=df_mcpe["high"],
                    low=df_mcpe["low"], close=df_mcpe["close"], name="K線", increasing_line_color=C_GREEN, decreasing_line_color=C_RED))
                fig_mcpe.add_trace(go.Scatter(x=pivots_df['date'], y=pivots_df['price'], mode="lines+markers",
                    line=dict(color=C_BLUE, width=2, dash="solid"), marker=dict(size=6, color="#e6edf3"), name="週期波段"))
                for i in range(1, len(pivots_df)):
                    prev_date = pivots_df['date'].iloc[i-1]; prev_price = pivots_df['price'].iloc[i-1]
                    curr_date = pivots_df['date'].iloc[i]; curr_price = pivots_df['price'].iloc[i]
                    days_chg = pivots_df['days'].iloc[i]; pct_chg = pivots_df['pct_chg'].iloc[i]
                    mid_date = prev_date + (curr_date - prev_date) / 2
                    color = C_GREEN if pct_chg > 0 else C_RED
                    text_str = f"{days_chg:.0f}日<br>{pct_chg:+.2f}%"
                    fig_mcpe.add_annotation(x=mid_date, y=prev_price + (curr_price - prev_price)/2, text=text_str, showarrow=False,
                        font=dict(color=color, size=11), bgcolor="rgba(13, 17, 23, 0.7)", bordercolor=color, borderwidth=1, borderpad=2)
                fig_mcpe.update_layout(height=600, paper_bgcolor=C_BG, plot_bgcolor=C_BG, font=dict(color="#e6edf3"),
...
with tab9:
    ...
    with beta_c1: beta_tk = st.text_input("輸入股票代碼 (Beta)", "MSTR", key="beta_tk").upper()
    with beta_c2: atr_mult = st.number_input("ATR 敏感度乘數", value=1.5, min_value=0.5, max_value=5.0, step=0.1, help="預設為 1.5 倍 ATR。數值越大，過濾的雜訊越多，只抓大波段。")
    with beta_c3:
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    if beta_run or beta_tk:
        ...
        if df_beta is not None and len(df_beta) > 60:
            ...
            with chart_col:
                fig_beta = go.Figure()
                fig_beta.add_trace(go.Candlestick(x=df_beta.index, open=df_beta["open"], high=df_beta["high"],
                    low=df_beta["low"], close=df_beta["close"], name="K線", increasing_line_color=C_GREEN, decreasing_line_color=C_RED))
                fig_beta.add_trace(go.Scatter(x=pivots_df['date'], y=pivots_df['price'], mode="lines+markers",
                    line=dict(color=C_PURPLE, width=2, dash="solid"), marker=dict(size=6, color="#e6edf3"), name="動態週期波段"))
                for i in range(1, len(pivots_df)):
                    prev_date = pivots_df['date'].iloc[i-1]; prev_price = pivots_df['price'].iloc[i-1]
                    curr_date = pivots_df['date'].iloc[i]; curr_price = pivots_df['price'].iloc[i]
                    days_chg = pivots_df['days'].iloc[i]; pct_chg = pivots_df['pct_chg'].iloc[i]
                    ...
                    color = C_GREEN if pct_chg > 0 else C_RED
                    text_str = f"{days_chg:.0f}日<br>{pct_chg:+.2f}%"
                    fig_beta.add_annotation(x=mid_date, y=prev_price + (curr_price - prev_price)/2, text=text_str, showarrow=False,
                        font=dict(color=color, size=11), bgcolor="rgba(13, 17, 23, 0.7)", bordercolor=color, borderwidth=1, borderpad=2)
                fig_beta.update_layout(height=600, paper_bgcolor=C_BG, plot_bgcolor=C_BG, font=dict(color="#e6edf3"),
                    xaxis_rangeslider_visible=False, margin=dict(l=10, r=10, t=30, b=10),
                    yaxis=dict(gridcolor="#21262d"), xaxis=dict(gridcolor="#21262d"))
            ...
            with data_col:
                if len(pivots_df) >= 3:
                    ...
                    st.markdown("""
                    <div style="background-color:#161b22; padding:15px; border-radius:10px; border:1px solid #30363d;">
                        <h4 style="color:#bc8cff; margin-top:0;">🧪 Pro Beta 動態引擎</h4>
                        <hr style="border-color:#30363d; margin: 10px 0;">
                    """, unsafe_allow_html=True)
                    ...
                    gann_status = f"貼近 {nearest_gann} 日窗口" if gann_diff <= 2 else f"距 {nearest_gann} 日窗口 {gann_diff:.0f} 日"
                    gann_color = C_ORANGE if gann_diff <= 2 else C_GREY

                    state_text = "上漲波段" if current_leg_type == 1 else "下跌波段"
                    state_color = C_GREEN if current_leg_type == 1 else C_RED
                    beta_row("市場結構狀態", state_text, state_color, True)
                    beta_row("當前運行天數", f"{current_days:.0f} 日")
                    beta_row("江恩時間窗", gann_status, gann_color, True)
                    st.markdown("<hr style='border-color:#30363d; margin: 10px 0;'>", unsafe_allow_html=True)
                    st.markdown("**🤖 AI 研判（預留接口）**")
                    st.caption("可接入 Perplexity API 做新聞面/籌碼面研判，及 Max Pain / IV 選擇權策略評估。")
                    st.button("🔗 連接 Perplexity AI 研判（開發中）", disabled=True)
                    st.markdown("</div>", unsafe_allow_html=True)

                    if gann_diff <= 2:
                        st.warning("⚠️ 當前波段運行天數貼近江恩關鍵時間窗，變盤機率上升。")
                    else:
                        st.info("ℹ️ 尚未進入關鍵江恩時間窗，可持續觀察。")
                else:
                    st.error("數據過少，無法形成足夠波段，請嘗試調整 ATR 乘數。")
        else:
            st.error("無法載入數據，請確認代碼是否正確。")
    st.divider()
    st.caption("Pro Beta 分頁為實驗性功能，數據來源：富途API + Yahoo Finance，僅供研究參考。")
