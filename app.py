...
with tab11:
    ...
    if st.button("🚀 執行多因子運算", type="primary", key="run_multifactor"):
        ...
        if factor_records:
            ...
            # 防崩潰設計：優先嘗試背景顏色渲染，若失敗則使用原生 column_config 格式化
            try:
                styled_df = f_df[cols_to_show].style.background_gradient(subset=['🏆 綜合得分'], cmap='RdYlGn')
                st.dataframe(styled_df, use_container_width=True, hide_index=True)
            except Exception:
                # 降級方案，無需安裝 matplotlib，直接使用原生設定
                st.dataframe(
                    f_df[cols_to_show],
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "🏆 綜合得分": st.column_config.ProgressColumn(
                            "🏆 綜合得分",
                            format="%.2f",
                            min_value=-5.0,
                            max_value=5.0
                        ...
                        "動能(6M%)": st.column_config.NumberColumn("動能(6M%)", format="%+.2f%%"),
                        "波動率(%)": st.column_config.NumberColumn("波動率(%)", format="%.2f%%")
                    }
                )
